...
import type { ClientMessage, ServerMessage, ServerTransport } from "@wc-bindable/remote";
...
const NODE_MODULES_ROOT = path.join(PKG_ROOT, "node_modules");

// Map URL prefix `/packages/<name>/dist/...` to a physical dist root.
// Stripe is served from this package's own build output; core/remote come
// from the installed `@wc-bindable/*` npm packages.
const PACKAGE_DIST_ROOTS: Record<string, string> = {
  stripe: path.join(PKG_ROOT, "dist"),
  core: path.join(NODE_MODULES_ROOT, "@wc-bindable", "core", "dist"),
  remote: path.join(NODE_MODULES_ROOT, "@wc-bindable", "remote", "dist"),
};
...
// Resolve a `/packages/<name>/dist/<rest>` URL to an absolute file path under
// the corresponding allowed root, or null on miss / traversal attempt.
function resolvePackageAsset(urlPath: string): string | null {
  const cleaned = urlPath.split("?")[0].split("#")[0];
  let decoded: string;
  try { decoded = decodeURIComponent(cleaned); } catch { return null; }
  if (decoded.includes("\0")) return null;
  const stripped = decoded.replace(/^\/+packages\/+/, "");
  const slashIdx = stripped.indexOf("/");
  if (slashIdx <= 0) return null;
  const pkgName = stripped.slice(0, slashIdx);
  const remainder = stripped.slice(slashIdx + 1);
  if (!remainder.startsWith("dist/")) return null;
  const root = PACKAGE_DIST_ROOTS[pkgName];
  if (!root) return null;
  return resolveSafeStaticPath(root, remainder.slice("dist/".length));
}
  ...
  const httpServer = http.createServer((req, res) => {
    try {
      ...
      if (req.method === "GET" && url.pathname.startsWith("/packages/")) {
        const file = resolvePackageAsset(url.pathname);
